...
its cause and a corrective action (NFR-04).
"""
...
def parse_real(text, name):
    ...
    stripped = text.strip()
    if stripped == "":
        raise F5Error("The value for " + name + " is empty.",
                      "enter a number such as 2, -0.5, or 10.")
    try:
        value = float(stripped)
    except ValueError:
        raise F5Error("'" + text + "' is not a number in decimal notation "
                      "for " + name + ".",
                      "enter a value such as 2, -0.5, or 10.")
    # Reject NaN and +/- infinity: value != value is true only for NaN.
    if value != value or value in (float("inf"), float("-inf")):
        raise F5Error("The value for " + name + " is not finite.",
...
class F5App:
    ...
    def __init__(self, root):
        self.root = root
        root.title("F5 Calculator  f(x) = a - b^x".replace("-", "\u00b7"))
        ...
        self.result = tk.Label(root, text="", font=mono, fg="#1a3d1a",
                               anchor="w")
        self.result.grid(row=4, column=0, columnspan=3, sticky="we",
                         pady=(12, 2))

        # Message line for errors (cause + corrective action).
        self.message = tk.Label(root, text="", font=small, fg="#a11", anchor="w",
                                wraplength=320, justify="left")
        self.message.grid(row=5, column=0, columnspan=3, sticky="we")

        # Controls.
        controls = tk.Frame(root)
        controls.grid(row=6, column=0, columnspan=3, pady=(12, 0), sticky="we")
