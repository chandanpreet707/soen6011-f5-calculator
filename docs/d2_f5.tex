...
%
% MAIN TRACK: 10 slides (title + 9), target under 6 minutes.
...
\begin{frame}{Problem 5: From-Scratch Strategy}
  ...
  \footnotesize The only built-in conversion left anywhere in the calculator is
  the \texttt{float()} that reads a text entry, which is an input function.
\end{frame}
...
\begin{frame}{Problem 5: Four Measured Improvements over D1}
  ...
  \raggedright On the last row Python's own \texttt{**} is $15\%$ wrong, because it
  passes through the subnormal range and this implementation does not.
\end{frame}
...
\begin{frame}{Problem 5: Exception Handling Design}
  ...
  \begin{itemize}
    ...
    \item \textbf{Two defects this design exposed, both now fixed.} D1 returned
          \texttt{inf} when a result left the representable range, so the
          interface showed ``f(x) = inf'': no cause, no corrective action,
          against NFR-02. And a zero multiplier short-circuited the domain
          rules, so $a = 0,\, b = -2,\, x = 0.5$ returned $0$ instead of being
          rejected under FR-04.
  \end{itemize}
\end{frame}
  ...
  \begin{columns}[T]
    \column{0.53\textwidth}
      ...
      \begin{itemize}
        ...
        \item The full history is in the repository; one representative message
              is enlarged here.
      \end{itemize}
...
\begin{frame}{Problem 7: Updated Requirements}
  ...
  \begin{columns}[T]
    ...
    \column{0.52\textwidth}
      \textbf{What changed.}
      \begin{itemize}
        \item \textbf{FR-07} (new) reject an entry that is not a decimal number.
        \item \textbf{FR-08} (new) reject an empty entry field.
        \item \textbf{FR-09} (new) reject an infinite or not-a-number entry.
        \item \textbf{FR-10} (new) Quit closes the window and ends the session.
        \item \textbf{NFR-04} (new) report a convergence failure without
              terminating.
        \item \textbf{DC-02, DC-03} (new) standard Python installation; no
              built-in or library function beyond input, output, arithmetic
              and interface design.
        \item \textbf{FR-05} trace corrected: was ``Completeness'', now
              \texttt{PP-01}, \texttt{G-01}.
        \item \textbf{FR-06} revised: ``prompt'' has no meaning in a GUI.
        \item \textbf{DC-01} revised: Tkinter GUI replaces the textual interface.
        \item \textbf{NFR-01} scope stated: six digits within the normal
              representable range.
        \item \textbf{NFR-02} wording unchanged, but now actually enforced.
      \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}{Appendix: Response to D1 Feedback}
  ...
  \begin{itemize}
    ...
    \item Malformed, empty and non-finite input, and quitting, are now
          requirements (FR-07, FR-08, FR-09, FR-10).
  \end{itemize}
\end{frame}
...
\begin{frame}{Appendix: Demonstration Plan (ninety seconds)}
  \begin{enumerate}
    ...
    \item Rejected input with a corrective message: $b = -2$, $x = 0.5$,
          then recovery and Quit.
  \end{enumerate}
  \medskip
  Each case shows one requirement working: FR-02, the range reduction, FR-05,
  and FR-04 with NFR-03 and FR-10.
\end{frame}
...
\begin{frame}{Appendix: Full Updated Requirements (3 of 3)}
  \scriptsize
  \begin{itemize}
    \item \textbf{NFR-01.} \emph{(scope stated)} The F5 Calculator shall
          compute results accurate to at least six significant digits for
          results within the normal representable range.
  ...
  \end{itemize}
  \medskip
  \textbf{Note 3 (NFR-01).} Below $2.2\times10^{-308}$ a double is subnormal
  and the format itself holds fewer than six significant digits, so the
  guarantee is stated for the normal range. Measured worst relative error
  inside that range is $5.2\times10^{-11}$ over 120{,}000 sampled inputs.
\end{frame}
...
\begin{frame}{Appendix: Traceability, Both Directions}
  ...
  covered by at least one requirement. This is the check the D1 marker asked for.
  \begin{center}
  \begin{tabular}{ll}
    \toprule
...
\begin{frame}{Appendix: Range-Safe Order of Multiplication}
  ...
  \begin{itemize}
    ...
    \item Reaching the subnormal range and coming back destroys most of the
          significant digits. This is how Python's \texttt{**} loses $15\%$ on
          $a = 6.7\times10^{248}$, $b = -6.73$, $x = -390$.
  \end{itemize}
  ...
  \textbf{Scope, stated honestly.} This covers every input for which
  $|b|^{n/2}$ is itself representable. Beyond that the calculator reports a
  \texttt{RangeError} rather than returning a wrong value: conservative there,
  never incorrect. Measured residue is about one case in 20{,}000 over inputs
  spanning 600 decades, none inside the persona's working range.
\end{frame}
...
\begin{frame}{Appendix: Verification Evidence}
  ...
  \begin{tabular}{lll}
    ...
    \midrule
    1. Accuracy     & 10 cases against \texttt{**}     & worst $4.6\times10^{-11}$ \\
    2. Rejection    & 6 inputs that must be refused    & all refused \\
    3. Range safety & 4 compensated-scale results      & all computed \\
    4. Convergence  & \texttt{ConvergenceError} forced & reported, no crash \\
    \bottomrule
  \end{tabular}
  \end{center}
  \medskip
  ...
  \footnotesize
  Ten accuracy cases, extended from the nine used in D1; the extra case is the
  extreme base $10^{15}$, which D1 could not compute at all, so the worst error
  is larger than D1's $2.4\times10^{-12}$ over a strictly harder set.
  The built-in \texttt{**} and \texttt{decimal} appear only in this script, as
  oracles; the calculator itself uses neither, so DC-03 holds.
\end{frame}
